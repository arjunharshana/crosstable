...
export function TournamentPanel({
  ...
  const handleUpdateResult = async (matchId: string, newResult: string) => {
    try {
      ...
      // Update the local state instantly (Optimistic UI!)
      setMatches((prev) =>
  ...
  return (
    <div className="flex flex-col gap-4 max-w-5xl mt-6 animate-in fade-in duration-300">
          ...
          >
            ...
            {!isByeMatch ? (
              <div className="flex gap-2 p-1 bg-black/40 rounded-lg border border-accent/10">
                ...
                <button
                  onClick={() => handleUpdateResult(match._id, "1/2-1/2")}
                  className={`px-4 py-2 rounded font-bold font-mono text-sm transition-all ${
