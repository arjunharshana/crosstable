...
export default function TournamentDetails() {
  ...
  const handleExportCSV = () => {
    ...
    const rows = standings.map((p) => [
      ...
      p.rating,
      p.score,
      p.tiebreaks.buchholzCut1,
      p.tiebreaks.buchholz,
      p.tiebreaks.sonnebornBerger,
      p.tiebreaks.progressiveScore,
      p.wins,
      p.draws,
      p.losses,
    ]);
    ...
    link.setAttribute(
      "download",
      `${tournament.name.replace(/\s+/g, "_")}_Final_Standings.csv`,
    );
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      // 3. Define page and table dimensions
      const margin = 40;
      ...
      // 4. Draw Header text
      page.drawText(`${tournament.name} - Final Standings`, {
        x: margin,
        y: currentY,
        size: 18,
        font: fontBold,
      ...
      page.drawText(
        `Generated by CrossTable on ${new Date().toLocaleDateString()}`,
        {
          x: margin,
          y: currentY,
          size: 10,
      ...
      // 5. Define Table Columns (X-coordinates for alignment)
      const columns = [
        { label: "Rank", x: margin },
        { label: "Player", x: margin + 40 },
        { label: "ELO", x: margin + 180 },
        { label: "Pts", x: margin + 230 },
        { label: "BH-1", x: margin + 270 },
        { label: "BH", x: margin + 310 },
        { label: "SB", x: margin + 350 },
        { label: "PS", x: margin + 390 },
        { label: "W-D-L", x: margin + 430 },
      ];

      const rowHeight = 20;

      const drawHeaders = (targetPage: PDFPage, startY: number) => {
      ...
      standings.forEach((p, index) => {
        ...
        const rowData = [
          ...
          p.rating.toString(),
          p.score.toFixed(1),
          p.tiebreaks.buchholzCut1.toString(),
          p.tiebreaks.buchholz.toString(),
          p.tiebreaks.sonnebornBerger.toString(),
          p.tiebreaks.progressiveScore.toString(),
      ...
      const link = document.createElement("a");
      link.href = url;
      link.download = `${tournament.name.replace(/\s+/g, "_")}_Final_Standings.pdf`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
    ...
    <div className="flex-1 overflow-y-auto bg-[radial-gradient(circle_at_top_right,_var(--tw-gradient-stops))] from-background/50 via-background to-background min-h-[calc(100vh-4rem)]">
      ...
      <header className="sticky top-0 z-20 p-6 md:p-8 flex flex-wrap items-center justify-between gap-6 bg-background/90 backdrop-blur-md border-b border-border">
        ...
        <div className="flex items-center gap-3">
          {/* EXPORT BUTTONS FOR COMPLETED TOURNAMENT */}
          {tournament.status === "Completed" && (
            <div className="flex gap-3 mr-4">
              <button
                onClick={handleExportCSV}
                className="flex items-center gap-2 px-4 py-2.5 bg-background border border-border rounded-xl text-sm font-bold text-foreground hover:border-accent transition-colors shadow-sm"
              >
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 bg-accent text-[#0B1120] rounded-xl text-sm font-bold hover:brightness-110 transition-colors shadow-sm"
              >
                <span className="material-symbols-outlined text-[18px]">
                  picture_as_pdf
                </span>
      ...
      <div className="p-6 md:p-8">
        {/* FIXED: Changed condition from "Ongoing" to !== "Upcoming" so it stays visible! */}
        {tournament.status !== "Upcoming" && (
          <div className="mb-8">
            <h3 className="text-xl font-serif font-bold text-foreground mb-4">
              {tournament.status === "Completed"
                ? "Final Results & Pairings"
                : "Pairings & Results"}
            </h3>
            <TournamentPanel
        ...
        {/* ... Rest of the component remains exactly identical (Overview, Arbiter Cards, Participants Table) ... */}
